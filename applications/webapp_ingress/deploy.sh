...

{
    ...
    while [ $i -le $MAX_INGRESS_COUNT ]; do
        ...
        loop=0        
        while [ $loop -lt 20 ]; do
            IP_ADDRESS=$(ssh -t -i $IDENTITY_FILE $USER_NAME@$MASTER_IP "kubectl get services -n $NAMESPACE_NAME -o json | jq --arg release $ingressName --arg component 'controller' '.items[] | select(.spec.selector.component == \$component) | select(.metadata.labels.release == \$release) | .status.loadBalancer.ingress[0].ip' | grep -oP '(\d{1,3}\.){1,3}\d{1,3}' || true")
    ...
    deploymentNames=($(ssh -t -i $IDENTITY_FILE $USER_NAME@$MASTER_IP "kubectl get services -n $NAMESPACE_NAME -o json | jq --arg type 'ClusterIP' '.items[] | select(.spec.type == \$type) | select(.spec.selector.component == null) | .metadata.name'"))
