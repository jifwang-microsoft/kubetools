...
then
    log_level -e "--upgrade-version is required"
    printUsage
fi
...
log_level -i "UPGRADE_VERSION:$UPGRADE_VERSION"
log_level -i "RESOURCE_GROUP:$RESOURCE_GROUP"
log_level -i "AZURE ENVIRONMENT:$AZURE_ENV"
log_level -i "AUTHENTICATION METHOD:$AUTH_METHOD"
...
if [ $CLIENT_SECRET == "" ] ; then
   log_level -e "Client Secret not found.Upgrade can not be performed"
    exit 1
fi
...
log_level -i "Upgrading of kubernetes cluster completed."
